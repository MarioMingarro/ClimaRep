...
#'
#' @description Combines multiple single-layer rasters (`tif`), typically outputs from `mh_rep` or `mh_rep_ch` for different input polygons, into a multi-layered `SpatRaster` specifically designed for RGB visualization.
#'
...
#'
#' This function handles inputs from both `mh_rep` (which primarily contains 'Retained' areas) and `mh_rep_ch` (which includes 'Retained', 'Lost', and 'Novel' areas) by counting the presence of
...
#'
#' @return Invisibly returns the resulting multi-layered `SpatRaster` object. This object has layers representing the cumulative counts for 'Lost' (first layer), 'Retained' (second layer), and 'Novel' (third layer) categories, designed for direct RGB plotting.
#'
#'  Additionally, a GeoTIFF file of the combined RGB raster named 'climarep_overlay.tif' is saved in a new 'overlay' subfolder within the `folder_path`. Returns `NULL` if no valid rasters are found or processed.
#'
#' @details
...
#' `mh_rep` (which yields only 'Retained' or 'Represented' areas, coded as `1`) and `mh_rep_ch` (which yields 'Retained' (`1`), 'Lost' (`2`), and 'Novel' (`3`) areas).
#'
...
#'  \item For every input raster, it creates a binary layer: `1` if the cell's value matches the target category (e.g., `2` for 'Lost'), and `0` otherwise.
...
#'
#' A new subfolder named `overlay/` will be created within the `folder_path` provided. The resulting three-layered RGB `SpatRaster` will be saved as 'climarep_overlay.tif'
